...
def patrol():
    ...
    if st == 200:
        ...
        for n in names:
            ...
            elif re.search(r'OTP@all|OTP@lgt|\u3010S-I|军令|奉\s*root', n, re.I):
...
def write_receipt(events, verdict='patrol-ok'):
    ...
    receipt = {
        'v': 'LGT-TOWER-01',
        'ts': ts,
        'events': events if events else [{'kind': 'patrol', 'ref': 'lgt-autonomy-02'}],
        'verdict': verdict,
        'repo': REPO
    }
    remote = f'receipts/tower/QT-{ts.replace(":","")}.json'
    text = json.dumps(receipt, ensure_ascii=False, indent=2)
    # Try to write via API first
    ...
    if old is not None:
        ...
        try:
            ...
            pass
    ok = put_file(remote, text, sha, f'lgt receipt {ts}')
    if not ok:
        # Fallback: write locally and commit via git
        os.makedirs('receipts/tower', exist_ok=True)
        with open(remote.replace('/', os.sep), 'w') as f:
            f.write(text)
        subprocess.run(['git', 'config', 'user.name', 'lgt-tower'], capture_output=True)
        subprocess.run(['git', 'config', 'user.email', 'lgt@chepin.ai'], capture_output=True)
        subprocess.run(['git', 'add', '-A'], capture_output=True)
        subprocess.run(['git', 'commit', '-m', f'lgt receipt {ts}', '-q'], capture_output=True)
        for i in range(3):
...
def self_cascade(pending):
    """Self-cascade: if there are pending items, wake self again after a delay.
    BURST-LOOP-01 fix: sleep CASCADE_SLEEP_S before dispatch (was zero-delay hot loop)."""
    ...
    st, _ = api('POST', 'dispatches', {
        'event_type': 'lgt-wake',
        'client_payload': {'src': 'lgt-tower', 'kind': 'self-cascade', 'pending': pending}
    })
    print(f'self-cascade dispatch: status={st}')

def mesh_wake():
    """Notify hub of tower activity."""
    ...
    req = urllib.request.Request(
        ...
        data=json.dumps({
            ...
            'client_payload': {'src': 'lgt-tower', 'kind': 'mesh-wake', 'sha': os.environ.get('GITHUB_SHA', '')}
        }).encode())
    try:
...
def save_state(st):
    ...
    put_file(STATE, json.dumps(st, ensure_ascii=False, indent=2), sha, 'lgt state')

if __name__ == '__main__':
    print(f'LGT-TOWER-01 starting at {datetime.datetime.now(datetime.timezone.utc).isoformat()}')
    st0 = load_state()
    seen = set(st0.get('seen', []))
    raw = patrol()
    # BURST-LOOP-01 fix: seen-set filter so sticky inbox/board items fire only once
