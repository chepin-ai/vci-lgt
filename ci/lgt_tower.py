...
def self_cascade():
    """Self-cascade: if there are pending items, wake self again."""
    events = patrol()
    if len(events) > 0:
        # There are pending items, trigger self again after a delay
        st, _ = api('POST', 'dispatches', {
            'event_type': 'lgt-wake',
            'client_payload': {'src': 'lgt-tower', 'kind': 'self-cascade', 'pending': len(events)}
        })
